...
function displayMatches(){
    ...
    const html = searchCities.map( find => {
        ...
        const highlightMatchedCity = find.city.replace(regex, `<span class='hl'>${searchedCity}</span>`); 
        const highlightMatchedState = find.state.replace(regex, `<span class='hl'>${searchedCity} </span>`); 
      return  `
        <li>
    ...
    //     return `
    //     <li>
    //       <span class='name'>${find.city}, ${find.state}</span>
    //       <span class='population'>${find.population}</span>
    //     </li>
    //   `;
    // };
